...
def _stream_worker(run_id: str, driver: TestUIDriver, fps: float, jpeg_quality: int, stop_event: threading.Event, stop_timeout: Optional[float] = None):
    """
    ...
    try:
        while not stop_event.is_set() and (stop_timeout is None or (time.time() - time_started) < stop_timeout):
            try:
                logging.info(f"Capturing screenshot for run {run_id}")
                png_bytes = driver.get_driver().get_screenshot_as_png()
                logging.info(f"Screenshot captured {run_id}")
            except Exception:
            ...
            try:
                ...
                logging.info(f"Screenshot transformed {run_id}")
            except Exception:
